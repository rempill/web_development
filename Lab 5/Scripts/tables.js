...
function renderClassicTable($container, data, sortKey, asc) {
    ...
    html += '</tbody></table>';
    $container.html(html);
    $container.find('th.sortable').each(function() {
...
$(function() {
    ...
    if (typeof menuTableData !== 'undefined') {
        const $classic = $('#classic-menu-table');
        if ($classic.length>0) renderClassicTable($classic, menuTableData, null, true);
    }
    // Vertical table (lots)
    if (typeof lotsTableData !== 'undefined') {
        const $vertical = $('#vertical-comparison-table');
        if ($vertical.length>0) renderVerticalTable($vertical, lotsTableData, null, true);
    }
});
